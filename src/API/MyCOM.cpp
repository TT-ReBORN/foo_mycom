...
HRESULT MyCOM::InitMyCOM() {
	...
	if (FAILED(hr)) {
		return LogError(hr, L"MyCOM => MyCOM::InitMyCOM", L"OleInitialize failed", true);
	}

	if (!GetModuleHandleEx(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS, reinterpret_cast<LPCTSTR>(static_cast<void*>(&InitMyCOM)), &hModule)) {
		OleUninitialize();
		return LogError(E_FAIL, L"MyCOM => MyCOM::InitMyCOM", L"Failed to get module handle", true);
	}

	hr = InitTypeLibrary(hModule);
	if (FAILED(hr)) {
		OleUninitialize();
		return LogError(hr, L"MyCOM => MyCOM::InitMyCOM", L"InitTypeLibrary failed", true);
	}

	hr = RegisterMyCOM();
	if (FAILED(hr)) {
		OleUninitialize();
		return LogError(hr, L"MyCOM => MyCOM::InitMyCOM", L"RegisterMyCOM failed", true);
	}
...
HRESULT MyCOM::InitTypeLibrary(HMODULE hModule) {
	...
	if (len == 0 || len >= MAX_PATH) {
		return LogError(E_FAIL, L"MyCOM => MyCOM::InitTypeLibrary", L"Failed to retrieve module path", true);
	}
	...
	if (FAILED(hr)) {
		return LogError(hr, L"MyCOM => MyCOM::InitTypeLibrary", L"LoadTypeLibEx failed", true);
	}
...
HRESULT MyCOM::RegisterMyCOM(std::wstring_view regMethod) {
	...
	if (!pClassFactory) {
		return LogError(E_OUTOFMEMORY, L"MyCOM => MyCOM::RegisterMyCOM", L"Failed to create class factory", true);
	}

	hr = CoRegisterClassObject(MyCOM_CLSID, pClassFactory, CLSCTX_INPROC_SERVER, REGCLS_MULTIPLEUSE, &dwRegister);
	if (FAILED(hr)) {
		return LogError(hr, L"MyCOM => MyCOM::RegisterMyCOM", L"CoRegisterClassObject failed", true);
	}
	...
	if (regMethod == L"RegEntry") {
		...
		if (FAILED(hr)) {
			...
			return LogError(hr, L"MyCOM => MyCOM::RegisterMyCOM", L"RegisterCLSID failed", true);
		}
	}
	else {
		...
		if (FAILED(hr)) {
			CoRevokeClassObject(dwRegister);
			return LogError(hr, L"MyCOM => MyCOM::RegisterMyCOM", L"HookCLSIDFromProgID failed", true);
		}
	}
...
HRESULT MyCOM::UnregisterMyCOM(std::wstring_view regMethod) {
	if (dwRegister != 0) {
		...
		if (FAILED(hr)) {
			return LogError(hr, L"MyCOM => MyCOM::UnregisterMyCOM", L"CoRevokeClassObject failed", true);
		}
...
HRESULT WINAPI MyCOM::MyCLSIDFromProgID(LPCOLESTR lpszProgID, LPCLSID lpclsid) {
	...
	return LogError(REGDB_E_CLASSNOTREG, L"MyCOM => MyCOM::MyCLSIDFromProgID", L"Class not registered", true);
}

HRESULT MyCOM::HookCLSIDFromProgID() {
	if (MH_Initialize() != MH_OK) {
		return LogError(E_FAIL, L"MyCOM => MyCOM::HookCLSIDFromProgID", L"MinHook initialization failed", true);
	}

	if (MH_CreateHook(&CLSIDFromProgID, &MyCLSIDFromProgID, reinterpret_cast<void**>(&original_CLSIDFromProgID)) != MH_OK) {
		MH_Uninitialize();
		return LogError(E_FAIL, L"MyCOM => MyCOM::HookCLSIDFromProgID", L"MinHook creation failed", true);
	}

	if (MH_EnableHook(&CLSIDFromProgID) != MH_OK) {
		MH_Uninitialize();
		return LogError(E_FAIL, L"MyCOM => MyCOM::HookCLSIDFromProgID", L"MinHook enable failed", true);
	}
...
HRESULT MyCOM::RegisterCLSID() {
	...
	if (FAILED(hr)) {
		return LogError(hr, L"MyCOM => MyCOM::RegisterCLSID", L"StringFromCLSID failed", true);
	}
	...
	if (FAILED(hr)) {
		return LogError(hr, L"MyCOM => MyCOM::RegisterCLSID", L"Failed to set registry for ProgID", true);
	}
	...
	if (len == 0) {
		return LogError(E_FAIL, L"MyCOM => MyCOM::RegisterCLSID", L"GetModuleFileNameW failed", true);
	}
	...
	if (FAILED(hr)) {
		return LogError(hr, L"MyCOM => MyCOM::RegisterCLSID", L"Failed to set InProcServer32 registry", true);
	}

	// Set ThreadingModel
	hr = SetRegistry(HKEY_CLASSES_ROOT, clsidKeyPath.c_str(), L"ThreadingModel", L"Apartment");
	if (FAILED(hr)) {
		return LogError(hr, L"MyCOM => MyCOM::RegisterCLSID", L"Failed to set ThreadingModel registry", true);
	}
...
HRESULT MyCOM::UnregisterCLSID() {
	...
	if (FAILED(hr)) {
		return LogError(hr, L"MyCOM => MyCOM::UnregisterCLSID", L"StringFromCLSID failed");
	}
	...
	if (result != ERROR_SUCCESS) {
		LogError(HRESULT_FROM_WIN32(result), L"MyCOM => MyCOM::UnregisterCLSID", L"Failed to delete CLSID registry key");
	}

	result = RegDeleteTree(HKEY_CLASSES_ROOT, L"MyCOM");
	if (result != ERROR_SUCCESS) {
		LogError(HRESULT_FROM_WIN32(result), L"MyCOM => MyCOM::UnregisterCLSID", L"Failed to delete MyCOM registry key");
	}
...
HRESULT MyCOM::SetRegistry(HKEY rootKey, const wchar_t* subKey, const wchar_t* valueName, const wchar_t* value) {
	...
	if (lResult != ERROR_SUCCESS) {
		return LogError(HRESULT_FROM_WIN32(lResult), L"MyCOM => MyCOM::SetRegistry", L"Failed to create/open registry key", true);
	}

	lResult = hKey.SetStringValue(valueName, value);

	if (lResult != ERROR_SUCCESS) {
		return LogError(HRESULT_FROM_WIN32(lResult), L"MyCOM => MyCOM::SetRegistry", L"Failed to set registry value", true);
	}
...
STDMETHODIMP MyCOM::QueryInterface(REFIID riid, void** ppvObject) {
	...
	*ppvObject = nullptr;
	return LogError(E_NOINTERFACE, L"MyCOM => MyCOM::QueryInterface", L"Interface not supported", true);
}
...
HRESULT MyCOM::InitTypeInfo() {
	if (MyCOM::typeInfo == nullptr && typeLib != nullptr) {
		...
		if (FAILED(hr)) {
			return LogError(hr, L"MyCOM => MyCOM::InitTypeInfo", L"GetTypeInfoOfGuid failed", true);
		}
	}
...
STDMETHODIMP MyCOM::GetTypeInfo(UINT iTInfo, LCID lcid, ITypeInfo** ppTInfo) {
	if (iTInfo != 0) {
		return LogError(DISP_E_BADINDEX, L"MyCOM => MyCOM::GetTypeInfo", L"Invalid type info index", true);
	}

	if (typeInfo == nullptr) {
		HRESULT hr = InitTypeInfo();
		if (FAILED(hr)) {
			return LogError(hr, L"MyCOM => MyCOM::GetTypeInfo", L"InitTypeInfo failed", true);
		}
	}
...
STDMETHODIMP MyCOM::GetIDsOfNames(REFIID riid, LPOLESTR* rgszNames, UINT cNames, LCID lcid, DISPID* rgDispId) {
	if (typeInfo == nullptr) {
		return LogError(E_NOTIMPL, L"MyCOM => MyCOM::GetIDsOfNames", L"Type info not initialized", true);
	}
...
STDMETHODIMP MyCOM::Invoke(DISPID dispIdMember, REFIID riid, LCID lcid, WORD wFlags, DISPPARAMS* pDispParams, VARIANT* pVarResult, EXCEPINFO* pExcepInfo, UINT* puArgErr) {
	if (typeInfo == nullptr) {
		return LogError(E_NOTIMPL, L"MyCOM => MyCOM::Invoke", L"Type info not initialized", true);
	}
...
STDMETHODIMP MyCOM::get_WindowWidth(long* pValue) {
	if (!pValue) {
		return LogError(E_POINTER, L"MyCOM => MyCOM::get_WindowWidth", L"Invalid pointer");
	}
	if (!MyComponent::Main()) {
		return LogError(E_UNEXPECTED, L"MyCOM => MyCOM::get_WindowWidth", L"Window not initialized");
	}

	*pValue = MyComponent::Main()->GetWindowWidth();
	return S_OK;
}
...
STDMETHODIMP MyCOM::put_WindowWidth(const VARIANT& newValue) {
	...
	MyComponent::Main()->SetWindowWidth(convertedValue);
	return S_OK;
}

STDMETHODIMP MyCOM::get_WindowHeight(long* pValue) {
	if (!pValue) {
		return LogError(E_POINTER, L"MyCOM => MyCOM::get_WindowHeight", L"Invalid pointer");
	}
	if (!MyComponent::Main()) {
		return LogError(E_UNEXPECTED, L"MyCOM => MyCOM::get_WindowHeight", L"Window not initialized");
	}

	*pValue = MyComponent::Main()->GetWindowHeight();
	return S_OK;
}
...
STDMETHODIMP MyCOM::put_WindowHeight(const VARIANT& newValue) {
	...
	MyComponent::Main()->SetWindowHeight(convertedValue);
	return S_OK;
}
#pragma endregion
...
STDMETHODIMP MyCOM::SetWindowSize(int width, int height) {
	if (!MyComponent::Main()) {
		return LogError(E_UNEXPECTED, L"MyCOM => MyCOM::SetWindowSize", L"Window not initialized");
	}
	...
	MyComponent::Main()->SetWindowSize(width, height);
	return S_OK;
}
...
STDMETHODIMP MyCOMClassFactory::QueryInterface(REFIID riid, void** ppvObject) {
	...
	*ppvObject = nullptr;
	return MyCOM::LogError(E_NOINTERFACE, L"MyCOM => MyCOMClassFactory::QueryInterface", L"Interface not supported", true);
}
...
STDMETHODIMP MyCOMClassFactory::CreateInstance(IUnknown* pUnkOuter, REFIID riid, void** ppvObject) {
	if (pUnkOuter != nullptr) {
		return MyCOM::LogError(CLASS_E_NOAGGREGATION, L"MyCOM => MyCOMClassFactory::CreateInstance", L"Aggregation not supported", true);
	}
